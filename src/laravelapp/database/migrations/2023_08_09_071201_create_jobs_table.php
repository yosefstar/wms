...
class CreateJobsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('jobs', function (Blueprint $table) {
            ...
            $table->integer('job_travel_cost')->length(10)->default(0)->comment('案件交通費');
            $table->integer('job_expense')->length(10)->default(0)->comment('案件経費');
            $table->dateTime('job_end_date')->nullable()->default(NULL)->comment('案件納品日');
            $table->dateTime('job_check_date')->nullable()->default(NULL)->comment('案件検修日');
